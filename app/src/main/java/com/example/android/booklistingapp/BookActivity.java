...
public class BookActivity extends AppCompatActivity implements LoaderCallbacks<List<Book>> {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        QUERY = intent.getStringExtra(MainActivity.EXTRA_KEY_WORD).replaceAll("\\s+","") + "&maxResults=10";
        Log.i("QUERY",QUERY);

        LinearLayout searchView = (LinearLayout) findViewById(R.id.search_view);
        if (intent.getStringExtra(MainActivity.EXTRA_BOOLEAN).equals("true")) {
    ...
    public Loader<List<Book>> onCreateLoader(int i, Bundle bundle) {
        Log.i("USGS_REQUEST_URL",USGS_REQUEST_URL+QUERY);
        // Create a new loader for the given URL
        return new BookLoader(this, USGS_REQUEST_URL + QUERY);
    }
